...
def creatpage():
    ...
    if data_product:
        ...
        # ======save the data====
        Product.to_csv('./Input/product.csv',index=False)
    ...
    if data: 
        ...
        with open('./Input/month_sales.csv','w',newline='') as fout:
            writer = csv.writer(fout)
            writer.writerow(dirty_data.columns)
            for d in dirty_data['Vente'].values:
                writer.writerow([d])
        second=pd.read_csv('./Input/month_sales.csv',sep=',',quotechar='|',encoding="ISO-8859-1")
